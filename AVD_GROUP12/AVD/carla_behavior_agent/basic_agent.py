...
from misc import (get_speed, is_within_distance,
                               get_trafficelement_trigger_location,
                               compute_distance)
...
class BasicAgent(object):
    ...
    def __init__(self, vehicle, opt_dict={}, map_inst=None, grp_inst=None):
        ...
        self._last_time_stop_signal = 0
        self._ignore_vehicles = False
        self._use_bbs_detection = False
        self._target_speed = 5.0
        self._sampling_resolution = 2.0
        self._base_tlight_threshold = 5.0  # meters
        self._base_tstop_threshold = 5.0   # meters
        ...
        if 'base_tstop_threshold' in opt_dict:
            self._base_tstop_threshold = opt_dict['base_tstop_threshold']
        ...
        self._stops_map = {}  # Dictionary mapping a stop sign to a wp corrspoing to its trigger volume location
        # ---------------------------------------------------------------------------------------------------------   
    ...
    def _affected_by_traffic_light(self, lights_list=None, max_distance=None):
        ...
        for traffic_light in lights_list:
            if traffic_light.id in self._lights_map:
                ...
                trigger_location = get_trafficelement_trigger_location(traffic_light)
    ...
    def _affected_by_stop_sign(self, stops_list=None, max_distance=None):
        ...
        Method to check if there is a stop sign affecting the vehicle.
            ...
            :param stops_list (list of traffic.sign with type "stop"): list containing stop_signs objects.
        ...
        if self._world.get_snapshot().timestamp.elapsed_seconds < self._last_time_stop_signal + self._stop_time:
            return (True, self._last_stop_sign)
        
        if not stops_list:                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 
            traffic_signs = self._world.get_actors().filter("*traffic.stop*")
            stops_list = []
            for sign in traffic_signs:
                if self._last_stop_sign is not None:
                    if sign.id != self._last_stop_sign.id:
                        stops_list.append(sign)
                else:
                    stops_list.append(sign)

        if not max_distance:
            max_distance = self._base_tstop_threshold
        ...
        for stop_sign in stops_list:
            if stop_sign.id in self._stops_map:
                ...
                trigger_location = get_trafficelement_trigger_location(stop_sign)
                trigger_wp = self._map.get_waypoint(trigger_location)
                self._stops_map[stop_sign.id] = trigger_wp
            ...
            if is_within_distance(trigger_wp.transform, self._vehicle.get_transform(), max_distance, [0, 90]):
                ...
                self._last_time_stop_signal = self._world.get_snapshot().timestamp.elapsed_seconds
            ...
            if not ego_wpt.is_junction or not target_wpt.is_junction:
                if target_wpt.road_id != ego_wpt.road_id or target_wpt.lane_id != ego_wpt.lane_id  + lane_offset:
                    ...
                    if str(target_wpt.lane_type) == "Shoulder":
                        ...
                        continue
                    elif target_wpt.road_id != next_wpt.road_id or target_wpt.lane_id != next_wpt.lane_id  + lane_offset:
                        continue
